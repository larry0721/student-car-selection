  ...
  return (
    <section className="rounded-lg border border-cyan-200/15 bg-cyan-200/[0.04] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.2)] md:p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-200">What would you like to ask?</p>
          <h2 className="mt-1 text-xl font-black tracking-tight text-white md:text-2xl">What else should we talk through?</h2>
        </div>
        <button
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.12fr)_minmax(280px,0.88fr)]">
        <div className="rounded-lg border border-white/10 bg-slate-950/40 p-4 md:p-5">
          <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">Why I’m recommending it</p>
          ...
          <div className="mt-4 rounded-lg border border-amber-200/15 bg-amber-200/10 px-3 py-3">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-amber-100/80">What worries me most?</p>
        ...
        <div className="grid gap-3">
          <div className="rounded-lg border border-white/10 bg-slate-950/30 p-4">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">What did I consider?</p>
          ...
          {narrative.nearWinner ? (
            <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
              <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">Which car almost won?</p>
          ...
          <div className="rounded-lg border border-cyan-200/15 bg-cyan-200/[0.07] p-4">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-cyan-100/80">What would I ask next?</p>
        ...
        {latestPlan?.preferenceLed && latestPlan.recalculationRequired ? (
          <div className="rounded-lg border border-amber-200/20 bg-amber-200/10 p-3 lg:col-span-2">
            <p className="text-sm font-bold leading-6 text-amber-50">
              This is preference-led exploration. My saved recommendation has not changed.
            </p>
            {latestPlan.nextAction?.includes("Would you accept") ? (
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  ...
                  onClick={() => recordPreferenceDiscovery("record_preference_discovery", "Yes, I’d accept that tradeoff")}
                  type="button"
                >
                  Yes, accept that
                </button>
        ...
        {session.entries.length > 1 ? (
          <details className="group rounded-lg border border-white/10 bg-slate-950/25 p-3 lg:col-span-2">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
              <span className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">What have we talked through?</span>
        ...
        {session.preferenceDiscoveries.length ? (
          <div className="rounded-lg border border-white/10 bg-slate-950/25 p-3 lg:col-span-2">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">What did I learn from this?</p>
